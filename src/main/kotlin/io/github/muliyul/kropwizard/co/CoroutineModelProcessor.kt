...
class CoroutineModelProcessor @Inject constructor(
	...
	private fun getContinuationTypeParameter(continuationType: Type): Type {
		...
		val lowerBound = if (typeArgument is WildcardType) typeArgument.lowerBounds[0] else typeArgument
		return if (lowerBound == Unit::class.java) Void.TYPE else lowerBound
	}

	private fun createClassWrapper(method: ResourceMethod) = ByteBuddy()
		...
		.withParameters(
			method.invocable.parameters
				.dropLast(1) // drop the Continuation param
				.map { it.type }
		)
		.intercept(
		...
		.let { fakeMethod ->
			method.invocable.definitionMethod.parameterAnnotations.dropLast(1) // drop Continuation param
